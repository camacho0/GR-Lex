...
def Genera(n,m,p,k):
	fila=np.random.randint(0,1001,size=m)
	name=str(k)
	#print(fila.shape)
	matriz =fila
	...
	#	print(medios)
		...
		if(Densidad(matriz)<=(p/100)):
			for j in range(m):
				...
				if(rd<=0.99):
					unos+=1
					g=np.random.randint(501,1001)
					#g=np.random.randint(50,101)
					fila[j]=g
				else:
					g=np.random.randint(1,501)
			...
			for j in range(m):
				rd=np.random.rand()
				if(rd<0.001):
					fila[j]=np.random.randint(1,501)
				else:
	...
	decimales=matriz/1000
	np.savetxt(name+".txt",decimales,delimiter=" ",fmt="%0.3f")
